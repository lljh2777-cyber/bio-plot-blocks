test_that("visual chart cards switch between volcano and scatter module stacks", {
  ...
  shiny::testServer(
    server,
    {
      ...
      initial_module_ids <- vapply(state$project$modules, `[[`, character(1), "module_id")
      expect_true("r.ggplot2.ggplot" %in% initial_module_ids)
      expect_true("r.ggplot2.geom_point" %in% initial_module_ids)
